...
# getConsulRaftPeers returns a list of the IP addresses of the Consul servers
# according to the /v1/operator/raft/configuration endpoint. We always query
# the local Consul agent.
function getConsulRaftPeers() {
  curl --silent "http://127.0.0.1:8500/v1/operator/raft/configuration" \
    | jq -M -r '.Servers[] | .Address' \
    | cut -d ':' -f1
}

# forceLeaveRaftPeer force leaves the given node from the Consul cluster.
#
# Parameters:
...
function forceLeaveRaftPeer() {
  ...
  node_id_to_leave=$(curl --silent "http://127.0.0.1:8500/v1/operator/raft/configuration" \
    | jq -M -r ".Servers[] | select(.Address == \"${node_address_to_leave}:8300\") | .Node")

  if [ ! -z "${node_id_to_leave}" ] ; then
    /usr/bin/consul force-leave "${node_id_to_leave}"
  fi
}

# forceLeaveOldServers compares the list of current Consul raft peers obtained
# via the /v1/operator/raft/configuration endpoint with the list of instances which
# are running or pending in the autoscaling group, and force leaves any servers
# which are not running or pending. We use the local Consul for queries and to
# carry out the force-leave operation.
#
# Parameters:
#     None
